- [How to address docker secret in kaniko](#how-to-address-docker-secret-in-kaniko)
- [install product via helm-chart on local k8 ](#install-via-helm-chart-on-local-k8)

# How to address docker secret in kaniko
Kaniko needs the docker registry creds etc to push the docker image. We can pass that as a kubernetes secret object and kanio can use that.
...
WHile creating secret , sometime u need to create in same namespace tekton-pipelines so the pipelines can read
